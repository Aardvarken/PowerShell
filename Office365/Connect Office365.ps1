...
# Connect Everything
#Connect-Office365 -Service Exchange, AzureAD, MSOnline, Teams, SecurityAndCompliance, SharePoint, SkypeForBusiness

# Azure commands only
#Connect-Office365 -Service AzureAD -MFA

# 365 Phone System commands only
...
# User Exit / Archive / Lock-out Commands;

# Clear Immutable IDs to break sync (after deleting/disabling in AD and syncing)
#$immutableUser = "danielle.schofield@sparrow.edu.au"
#Set-MSOLUser -UserPrincipalName $immutableUser -ImmutableID "$null"
#Get-MsolUser -UserPrincipalName $immutableUser | Select UserPrincipalName, ImmutableID

# SharePoint commands only - REPLACE OrgName with the prefix from OrgName.sharepoint.com
#Connect-Office365 -Service AzureAD, SharePoint -SharePointOrganizationName OrgName -MFA

# Sign-out 365 services (Requires Sharepoint connection above)
#Revoke-SPOUserSession - User email@domain.com -Confirm:$false

# Sign-out Azure services (Requires Azure connection above)
#Get-AzureADUser -searchstring email@domain.com | Revoke-AzureADUserAllRefreshToken

# Set a password to never expire - ONLY WORKS ON NON-SYNCED ACCOUNTS (obviously, but just gotta say to remind you)
# https://docs.microsoft.com/en-us/office365/admin/add-users/set-password-to-never-expire?view=o365-worldwide for more info I guess
#Set-AzureADUser -ObjectId email@domain.com -PasswordPolicies DisablePasswordExpiration